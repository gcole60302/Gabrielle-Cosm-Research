...
#SN_ALL_BIN is an array of the S/N ratio of the clusters in a specific bin
    ...
    return R_Stand, BIN31 #arrays are largest radial value to smallest and smallest temp dec to largest temp dec
...
def SN(num_sim,num_clus, num_reps):
    R_Stand = FULLMAP(num_clus)[0]

    DATA = np.zeros((num_reps,num_sim, len(R_Stand))) #len(AVG_R[AVG_R!=0]
    for i in range(num_reps):
        for j in range(num_sim):
            DATA[i,j] = FULLMAP(num_clus)[1]
    DATA = DATA.T
    print DATA

    VAR = np.zeros((num_reps, len(R_Stand)))
    MEAN = np.zeros((num_reps, len(R_Stand)))
    SD = np.zeros((num_reps, len(R_Stand)))

    for j in range(num_reps):
        for i in range(len(R_Stand)):
            VAR[j,i] = np.var(DATA[i,:,j])
            MEAN[j,i] = np.average(DATA[i,:,j])
            SD[j,i] = np.std(DATA[i,:,j])

    MV = MEAN/VAR
    MS = MEAN/SD
    MV1 = MV.T
    MS1 = MS.T
    MV_var = np.zeros(len(R_Stand))
    MV_sd = np.zeros(len(R_Stand))
    MS_var = np.zeros(len(R_Stand))
    MS_sd = np.zeros(len(R_Stand))
    for i in range(len(R_Stand)):
        MV_var[i] = np.var(MV1[i])
        MV_sd[i] = np.std(MV1[i])
        MS_var[i] = np.var(MS1[i])
        MS_sd[i] = np.std(MS1[i])
    
    plt.figure()
    plt.title('20 Simulations, 3 Clusters Each, 2 reps')
    plt.xlabel('Radial Distance')
    plt.ylabel('Mean/Variance')
    plt.errorbar(R_Stand, MV.sum(axis=0)/num_reps, yerr = MV_var, fmt='o')

    plt.figure()
    plt.title('20 Simulations, 3 Clusters Each, 2 reps')
    plt.xlabel('Radial Distance')
    plt.ylabel('Mean/Variance')
    plt.scatter(R_Stand, MV[0])

    plt.figure()
    plt.title('20 Simulations, 3 Clusters Each, 2 reps')
    plt.xlabel('Radial Distance')
    plt.ylabel('Mean/Variance')
    plt.scatter(R_Stand, MS[0])
    
    plt.figure()
    plt.title('20 Simulations, 3 Clusters Each, 2 reps')
    plt.xlabel('Radial Distance')
    plt.ylabel('Mean/Standard Deviation') 
    plt.errorbar(R_Stand, MS.sum(axis=0)/num_reps, yerr = MS_var, fmt='o')

    plt.figure()
    plt.title('20 Simulations, 3 Clusters Each, 2 reps')
    plt.xlabel('Radial Distance')
    plt.ylabel('Mean')
    plt.scatter(R_Stand, (MEAN.sum(axis=0)/num_reps))
    return
