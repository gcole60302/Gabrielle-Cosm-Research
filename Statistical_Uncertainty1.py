...
def NMap():
    ...
    for i in range(SIZE*4):
        for j in range(SIZE*4):
            N1[i,j] = np.random.normal(0.0, 8.0)
...
######################################################################            
    for k in range(1): #change back to 5 later to cover all bins
        A = ALL_BINS[k]
        vects1 = np.linspace(0,SIZE1, SIZE1*4+1)
        x1,y1 = np.meshgrid(vects1, vects1)
        DIST = np.zeros((SIZE2,SIZE1))
...
#Average all values in a given radial range
        for j in range(SIZE1*SIZE2):
            AVG_T[j] = ARRAY_AVG(AVG_R_T[j])
            VAR_T[j] = ARRAY_VAR(AVG_R_T[j])
        ...
        BINS = 456 #456: 38,12,2,228,4,114,8,57,19,24
        FACTOR = TOTAL_NUM_DATA/BINS
        
        #BINNED_RADIAL[k] = R_Stand.reshape((BINS,FACTOR)).mean(axis=1)
        #BINNED_T_T[k] = T_Avg.reshape((BINS,FACTOR)).mean(axis=1)
        #BINNED_VAR[k] = Var.reshape((BINS,FACTOR)).mean(axis=1)

        #plt.figure()
        #plt.scatter(BINNED_RADIAL[k], BINNED_T_T[k])
        #plt.title('Comptonization Factor as a Function of Radius (25 Clusters)', fontsize=19)
        ...
        #plt.figure()
        #plt.errorbar(BINNED_RADIAL[k], BINNED_T_T[k], yerr = BINNED_VAR[k], fmt = 'o') 
        #plt.title('Variance as a Function of Radius (25 Clusters)', fontsize=19)
        #plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
        #plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)

        #plt.figure()
        #plt.scatter(BINNED_RADIAL[k], (BINNED_T_T[k])/(BINNED_VAR[k]))
        #plt.title('Signal Over Variance as a Function of Radius (25 Clusters)', fontsize=19)
        #plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
        #plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)
    return #BINNED_RADIAL,BINNED_T_T, BINNED_VAR
...
def STAT_UNCER_ALL(num_sim,num_clus, bin_num):
    R_All = STAT_UNCERT(num_clus)[0]
    T_All = STAT_UNCERT(num_clus)[1]
    ...
    plt.figure()
    plt.scatter(R, MEAN)
    plt.title('Comptonization as a Function of Radius (25 Simulations of 15 Clusters Each)', fontsize=19)
    plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
    plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)
    ...
    plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
    plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)
    plt.annotate('Variance Simulation to Simulation are Error Bars', xy=(5, 500))

    plt.figure()
    plt.scatter(R, (MEAN)/(VAR))
    plt.title('Signal Over Variance as a Function of Radius (25 Simulations of 15 Clusters Each)', fontsize=19)
    plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
    plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)
