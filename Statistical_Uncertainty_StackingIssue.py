...
###################################################################
#Here we define a function to convert R500 to arcmin
...
#Populate T_at_R with temp values corresponding to radial distances
    interpol = scipy.interpolate.interp1d(R,T, kind='cubic', bounds_error=False, fill_value=0) 
    for i in range(2*Dim*4+1):
        for j in range(2*Dim*4+1):
...
#2 is for redshift, 3 is for mass
        ...
        ALL_BINS[3] = BIN4
        ALL_BINS[4] = BIN5
        BINNED_RADIAL = np.zeros((5,456)) #456: 38,12,2,228,4,114,8,57,19,24
        BINNED_T_T = np.zeros((5,456)) #456: 38,12,2,228,4,114,8,57,19,24
        BINNED_VAR = np.zeros((5,456)) #456: 38,12,2,228,4,114,8,57,19,24
        for k in range(1):
            A = ALL_BINS[k]
            vects1 = np.linspace(0,SIZE1, SIZE1*4+1)
            x1,y1 = np.meshgrid(vects1, vects1)
            ...
            for i in range(SIZE1*SIZE2):
                if R_T[i,0] <= .25:
                    BINT[0,i] = R_T[i,1]
                    BINR[0,i] = R_T[i,0]
                elif R_T[i,0] > .25 and R_T[i,0] <= 0.66:
                    BINT[1,i] = R_T[i,1]
                    BINR[1,i] = R_T[i,0]
                elif R_T[i,0] > 0.66 and R_T[i,0] <= 1.0:
                    BINT[2,i] = R_T[i,1]
                    BINR[2,i] = R_T[i,0]
                    ...
                    BINT[16,i] = R_T[i,1]
                    BINR[16,i] = R_T[i,0]
                elif R_T[i,0] > 10.0 and R_T[i,0] <= 10.3:
                    BINT[17,i] = R_T[i,1]
                    BINR[17,i] = R_T[i,0]
                elif R_T[i,0] > 10.3 and R_T[i,0] <= 10.6:
                    BINT[18,i] = R_T[i,1]
                    BINR[18,i] = R_T[i,0]
                elif R_T[i,0] > 10.6 and R_T[i,0] <= 10.9:
                    BINT[19,i] = R_T[i,1]
                    BINR[19,i] = R_T[i,0]
                elif R_T[i,0] > 10.9 and R_T[i,0] <= 11.0:
                    BINT[20,i] = R_T[i,1]
                    BINR[20,i] = R_T[i,0]
                elif R_T[i,0] > 11.0:
                    BINT[21,i] = R_T[i,1]
            ...
            #print "Avg Var is", np.mean(Var)
            #print "S to N is", np.max(T_Avg/Var)
            
            #plt.figure()
            #plt.errorbar(R_Stand, T_Avg, yerr = Var, fmt = 'o')
            #R500 = R500ToArcmin(0.5, 3.2e14)
            #plt.axvline(R500)
        ...
        for k in range(1):
            ...
            for i in range(n):
                C[i] = MAP(Z, M500)
                N[i] = NMap()[0:SIZE,0:SIZE]
            C = (C.sum(axis=0)) #/ (n)
            
            N = (N.sum(axis=0)) #/ (np.sqrt(n))
            SUM =  N + C
            
            vects1 = np.linspace(0,SIZE, SIZE*4+1)
            x1,y1 = np.meshgrid(vects1, vects1)
            DIST = np.zeros((SIZE,SIZE))
            for i in range(SIZE):
                for j in range(SIZE):
                    DIST[i,j] = np.sqrt(((x1[SIZE/2,SIZE/2] +(1/8.)) - (x1[i,j] + (1/8.)))**2 +((y1[SIZE/2,SIZE/2] +(1/8.)) - (y1[i,j] + (1/8.)))**2)
            LIST1 = DIST.reshape(SIZE*SIZE,1)
            LIST2 = SUM.reshape(SIZE*SIZE,1)
            R_T = np.zeros((SIZE*SIZE,2))
            BINS = 22
            BINT = np.zeros((BINS, SIZE*SIZE))
            BINR = np.zeros((BINS, SIZE*SIZE))
            ...
            #R500 = R500ToArcmin(0.5, 3.2e14)
            #plt.axvline(R500)
            #plt.title('Mean as a Function of Radius (500 Clusters Variance as Errorbars)(Noise factor 1/sqrt)', fontsize=19)
            #plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
            #plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)
            ...
            #R500 = R500ToArcmin(0.5, 3.2e14)
            #plt.axvline(R500)
            #plt.title('Signal Over Variance as a Function of Radius (500 Clusters)(Noise factor 1/sqrt)', fontsize=19)
            #plt.xlabel(r'$\mathrm{Radial}\/\mathrm{Distance}\/\mathrm{(Arcmin)}$', fontsize=19)
            #plt.ylabel(r'$\mathrm{Comptonization,}\/\mathrm{y}\/\mathrm{(\mu K)}$',fontsize=19)
...
def SNR(bin1, bin2, bin3):
    R = STAT_UNCERT(1)[0]
    NUM_CLUST = np.arange(1,100,2)

    DATA = np.zeros((len(NUM_CLUST), len(R)))
    
    for i in range(len(NUM_CLUST)):
        DATA[i] = STAT_UNCERT(NUM_CLUST[i])[1]

    DATA = DATA.T
    BINS = np.zeros(3)
    BINS[0],BINS[1],BINS[2] = bin1, bin2, bin3
    
    for i in range(len(BINS)):
        plt.figure()
        plt.scatter(NUM_CLUST, DATA[BINS[i]])
        plt.title(np.str(BINS[i]), fontsize=19)
        plt.xlabel(r'$\mathrm{Signal}\/\mathrm{to}\/\mathrm{Noise}\/\mathrm{(Ratio)}$', fontsize=19)
        plt.ylabel(r'$\mathrm{Number}\/\mathrm{of}\/\mathrm{Clusters}$',fontsize=19)
